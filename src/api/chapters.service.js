...
async function notesRequest(pageId, method, suffix = '', body) {
  const paths = [
    `/pages/${pageId}/notes${suffix}`,
    `/chapters/pages/${pageId}/notes${suffix}`,
  ]
  let lastErr
...
export const chaptersService = {
  create({ series_id, chapter_number, title }) {
    return http.post('/chapters', { series_id, chapter_number, title }).then(unwrap)
  },
